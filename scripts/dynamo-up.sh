...
WAIT="${WAIT:-true}"
TIMEOUT="${TIMEOUT:-600}"
POD_SELECTOR="${POD_SELECTOR:-dynamo-component=${WORKER_NAME}}"
...
Environment overrides:
  ...
  POD_SELECTOR=dynamo-component=\$WORKER_NAME
                      (label selector for the worker pod)
...
if [[ "$WAIT" == "true" ]]; then
  ...
  echo "  kubectl -n ${NAMESPACE} get pods -l ${POD_SELECTOR}"
fi
