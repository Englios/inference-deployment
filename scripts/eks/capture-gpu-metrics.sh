...
kubectl -n "${PROM_NAMESPACE}" get svc "${PROM_SERVICE}" >/dev/null 2>&1 || {
  echo '{"status":"skipped","reason":"prometheus_service_not_found"}' > "${GPU_PATH}"
  exit 0
}

kubectl -n "${PROM_NAMESPACE}" port-forward svc/"${PROM_SERVICE}" "${PROM_LOCAL_PORT}:9090" >/tmp/gpu-metrics-port-forward.log 2>&1 &
pf_pid=$!
...
if [[ "${ready}" != "true" ]]; then
  echo '{"status":"skipped","reason":"prometheus_not_ready"}' > "${GPU_PATH}"
  exit 0
fi
